// Authoritative DateTime Service for SOMYA AI
// Real system/browser runtime clock with Asia/Kolkata timezone priority
...
/**
 * Returns the authoritative live date & time dynamically from the runtime clock.
...
export function getLiveDateTime(timezone: string = 'Asia/Kolkata'): DateTimeInfo {
  ...
  // Validate timezone or fallback to Asia/Kolkata
...
/**
 * Hook to get a continuously updating live clock every second
 * Automatically rolls over at midnight seamlessly.
 */
export function useLiveDateTime(timezone: string = 'Asia/Kolkata') {
  const [dateTime, setDateTime] = useState<DateTimeInfo>(() => getLiveDateTime(timezone));

  useEffect(() => {
    // Immediate sync
    setDateTime(getLiveDateTime(timezone));
...
export function detectDateTimeQuery(text: string): DateTimeQueryType | null {
  ...
  // Day of the week queries (Day / Din / Vaar)
  const dayPatterns = [
...
/**
 * Generates a natural, warm, conversational response for date/time queries
 * strictly using the verified runtime DateTimeInfo.
 */
export function generateDateTimeReply(
  ...
  // Detect script/language preference
  ...
  // 3. NATURAL HINGLISH RESPONSES (DEFAULT FOR BILINGUAL ASSISTANT)
  switch (type) {
    case 'DATE_ONLY':
      return {
